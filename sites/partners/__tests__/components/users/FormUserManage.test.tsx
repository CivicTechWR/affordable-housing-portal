...
import { Listing, User } from "@bloom-housing/shared-helpers/src/types/backend-swagger"
...
describe("<FormUserManage>", () => {
  it("shows only Admin, Partner, and User for admins and submits a User invite with no role flags", async () => {
    ...
    server.use(
      rest.post("http://localhost/api/adapter/user/invite", async (req, res, ctx) => {
        requestSpy(await req.json())
        return res(ctx.json({ success: true }))
      })
    )

    renderForm(adminUserWithJurisdictions, { onDrawerClose })
  ...
  it("shows only Partner and User for jurisdictional admins", async () => {
    server.use(
  ...
  it("maps existing no-role users back to the User dropdown option", async () => {
    server.use(
  ...
  it("updates partner users without changing the simplified role behavior", async () => {
    ...
    server.use(
      rest.put("http://localhost/api/adapter/user/%7Bid%7D", async (req, res, ctx) => {
        requestSpy(await req.json())
        return res(ctx.json({ success: true }))
      })
    )
