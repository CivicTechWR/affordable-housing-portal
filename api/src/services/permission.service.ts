...
export class PermissionService {
  ...
  */
  async addUserPermissions(enforcer: Enforcer, user: User): Promise<Enforcer> {
    // Resolve singleton jurisdiction ID for casbin rules
    const singletonJuris = await this.prisma.jurisdictions.findFirst({
      select: { id: true },
    });
    const userJurisdictionId = singletonJuris?.id;
