...
import { CustomListingFeatureService } from '../../../src/services/custom-listing-feature.service';
...
describe('Testing custom feature service', () => {
  let service: CustomListingFeatureService;
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [CustomListingFeatureService, PrismaService],
    }).compile();

    service = module.get<CustomListingFeatureService>(CustomListingFeatureService);
  ...
  it('testing findCategorizedByScope()', async () => {
    const date = new Date();
    ...
    prisma.customListingFeatures.findMany = jest
      .fn()
      .mockResolvedValue(mockedFeatures);

    const result = await service.findCategorizedByScope(
      CustomListingScope.PROPERTY,
    );

    expect(result).toHaveLength(2);

    const propertyAmenities = result.find(
      (c) => c.name === 'PROPERTY AMENITIES',
    );
    expect(propertyAmenities).toBeDefined();
    expect(propertyAmenities.features).toHaveLength(2);
    expect(propertyAmenities.features[0].key).toBe('elevator');
    expect(propertyAmenities.features[1].key).toBe('braille_signage');

    const entryExterior = result.find((c) => c.name === 'ENTRY & EXTERIOR');
    expect(entryExterior).toBeDefined();
    expect(entryExterior.features).toHaveLength(1);
    expect(entryExterior.features[0].key).toBe('accessible_parking');

    expect(prisma.customListingFeatures.findMany).toHaveBeenCalledWith({
      where: { scope: CustomListingScope.PROPERTY },
      orderBy: { category: 'asc' },
    });
  });

  it('testing findCategorizedByScope() with no results', async () => {
    prisma.customListingFeatures.findMany = jest.fn().mockResolvedValue([]);

    const result = await service.findCategorizedByScope(
      CustomListingScope.UNIT,
    );

    expect(result).toEqual([]);

    expect(prisma.customListingFeatures.findMany).toHaveBeenCalledWith({
      where: { scope: CustomListingScope.UNIT },
      orderBy: { category: 'asc' },
    });
  });
});
