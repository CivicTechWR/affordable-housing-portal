...
describe('Testing listing csv export service', () => {
  ...
  describe('createCsv', () => {
    ...
    it('should create the listing csv with marketing type seasons', async () => {
      ...
      // Validate first row
      expect(content).toContain('"listing1-ID"');
      expect(content).toContain('"Under Construction","Summer","2025"');
      expect(content).toContain(
        '"https://www.example.com/building-criteria.pdf"',
      );
      expect(content).not.toContain('sample deposit helper text');
    ...
    it('should create the listing csv with marketing type months', async () => {
      ...
      // Validate first row
      expect(content).toContain('"listing1-ID"');
      expect(content).toContain('"Under Construction","July","2025"');
      expect(content).not.toContain('sample deposit helper text');
      expect(content).not.toContain('sample costs not included');
    });
    it('should create the listing csv with parking types column', async () => {
      ...
      // Validate first row
      expect(content).toContain('"listing1-ID"');
      expect(content).toContain('Parking Types');
      expect(content).toContain('"sample smoking policy"');
      expect(content).not.toContain('sample deposit helper text');
      expect(content).not.toContain('sample costs not included');
    });
